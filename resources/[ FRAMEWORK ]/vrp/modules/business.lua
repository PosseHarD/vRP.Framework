			...
			menu["Investir"] = { function(player,choice)
				...
				if amount > 0 then
					if vRP.tryPayment(user_id,amount) then
						...
						TriggerClientEvent("Notify",player,"sucesso","Investiu <b>$"..vRP.format(amount).." dólares</b> e adicionou <b>$"..vRP.format(parseInt(amount*0.5)).."</b> ao capital.",8000)
					else
						TriggerClientEvent("Notify",player,"negado","Dinheiro insuficiente.",8000)
					end
				end
			end }
			...
			menu["Lavagem"] = { function(player,choice)
				...
				if parseInt(amount) > 0 and parseInt(amount) <= restante then
					if vRP.tryGetInventoryItem(user_id,"dinheirosujo",parseInt(amount)) then
						...
						vRP.giveDinheirama(user_id,parseInt(amount*("0."..random)))
						TriggerClientEvent("Notify",player,"sucesso","Lavou <b>$"..vRP.format(amount).." dólares sujos</b> e recebeu <b>$"..vRP.format(parseInt(amount*("0."..random))).." dólares</b> limpos.",8000)
					else
						TriggerClientEvent("Notify",player,"negado","Capital insuficiente.",8000)
					end
				end
			end }
		else
			menu["Comprar"] = { function(player,choice)
				...
				if ok then
					if vRP.tryPayment(user_id,999999999) then
						...
						TriggerClientEvent("Notify",player,"sucesso","Compra concluida.",8000)
